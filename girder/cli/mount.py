...
class ServerFuse(fuse.Operations):
    ...
    def __init__(self, stat=None, flatten=False, options=None):
        """
    ...
    def _get_path(self, path):
        ...
        try:
            ...
            resource = path_util.lookUpPath(
                path.rstrip('/'), filter=False, force=True, flatten=self.flatten)
...
@click.option(
    ...
    help='Lazy unmount.')
@click.option(
    '-f', '--flatten', is_flag=True, default=False,
...
def mountServer(path, database=None, fuseOptions=None, quiet=False, verbose=0,
    ...
    opClass = ServerFuse(stat=os.stat(path), flatten=flatten, options=options)
    Setting().set(SettingKey.GIRDER_MOUNT_INFORMATION,
                  {'path': path, 'mounttime': time.time(), 'flatten': bool(flatten)})
